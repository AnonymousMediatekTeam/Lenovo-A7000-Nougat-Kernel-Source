...
extern bool bat_update_sleep_timeout;
...
//lenovo-sw mahj2 modify at 20141126 End
	...
	/*lenovo sw: AIOROW-3569 begin*/
	if (bat_meter_timeout == KAL_TRUE || bat_spm_timeout == TRUE || bat_update_sleep_timeout == true) {
	/*lenovo sw: AIOROW-3569 end*/
		...
		if (bat_spm_timeout == true) {
			...
			BMT_status.UI_SOC = battery_meter_get_battery_percentage();
               ...
               /* lenovo-sw zhangrc2 optim ui_soc update time in long sleep begin 2014-01-16 */  
		battery_xlog_printk(BAT_LOG_CRTI,"battery xuwei mt_battery_GetBatteryData\n");
		bat_meter_timeout = KAL_FALSE;
		bat_spm_timeout = FALSE;
		/*lenovo sw: AIOROW-3569 begin*/
		bat_update_sleep_timeout = false ;
		/*lenovo sw: AIOROW-3569 end*/
	} else {
		if (previous_SOC == -1)
			SOC = battery_meter_get_battery_percentage();
...
//lenovo-sw mahj2 modify at 20141126 Begin
    ...
    bat_vol = battery_meter_get_battery_voltage(KAL_TRUE);
    	battery_xlog_printk(BAT_LOG_CRTI, "Vbat=(%d)\n",bat_vol);
#endif
//lenovo-sw mahj2 modify at 20141126 End
...
static void battery_timer_resume(void)
{
	...
	/*lenovo sw: AIOROW-3569 begin*/
	if(is_pcm_timer_trigger == KAL_TRUE || bat_spm_timeout || bat_update_sleep_timeout == true)
	{
	/*lenovo sw: AIOROW-3569 end*/
		mutex_lock(&bat_mutex);
		BAT_thread();
