...
//lenovo jixu add begin
	#if defined(CONFIG_LENOVO_BACKLIGHT_SLOPE)
	{
		unsigned int real_level = level;
		if(level ==0)//(0,0),(10,10)
		real_level = (unsigned long)level;
		else if(level >0 && level <=10)//(0,0),(5,10)
		real_level = (unsigned long)level/2 + 3;
	...
	printk("[JX]do backlight levle slope,level=%u, slope_level=%u \n",level,real_level);	
		level =  real_level;
	}
	#endif
//lenovo jixu add end
